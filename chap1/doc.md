...
실행하곤한다. (nw는 no-window의 약자로 쉽게 기억할수 있다.) 터미널에 보이는
...
된다. \x{C-x b}를 실행하면 "Switch to buffer" 프롬프트가 미니버퍼에 나타난것을
...
\*scr[TAB]으로 자동완성해서 scratch 버퍼를 선택하자. 그리고 첫 elisp 프로그램을
만들어 보자.
...
문자열을 인자로하여 호출한 것이다. 사실 이것이 elisp의
syntax의전부이다. elisp에서는 일반 imperative 언어에서 특별하게 여겨지는 표현식,
...
있다.
 ...
 - built-in 함수: c로 구현된 함수
   이멕스는 c로 구현된 (기본이 되는) built-in 함수들과 elisp으로 구현된
...
자 넘어가기 전에 또 한번 새로운 용어들을 살펴 보자.
 ...
 - command & interactive: 사용자에게 직접적으로 노출된 함수들
   elisp으로 호출 할 수 있는 많은 함수들 중 사용자에게 명시적으로 노출된
...
이제는 많이 익숙해져서 도움말에 대한 해석은 뒤로하고 재미있는 elisp의 용어 하나를
알아보고 넘어가자. elisp은 interpreter로 해석이 되며 (컴파일 과정이 따로 필요
