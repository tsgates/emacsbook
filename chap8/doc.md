...
이유해서 이맥스를 배우기에 가장 쉬운 방법이 1장, 2장을 통해 설명한 도움말에 쉽게
접근하는 것이다.
...
있을것이다. 신기하게도 `man`에서는 그러한 기능이 없다.  이맥스는 Lisp으로 짜여진
더욱 편하고, 아름다운? `woman`이 있다. 관심있는 독자는 실행해 보고, 문서가 눈에
더 잘들어오도록 설정해 보기바란다.

자 그럼! 사용자가 이맥스의 \f{man} 리습함수를 호출 하면, 이맥스에서는 어떠한 일이
일어나고 있는 것일까?
...
보는것이 맞을 것이다. 이맥스의 `man`에서 하는 일은 SEE ALSO 섹션에 나열되어있는
...
이렇게 간단하게 프로그램을 호출 할수 있다. 첫 인자 '이름'은 이맥스 내부의 PID라고
...
사용한다. 왜 버퍼를 사용할까? 이미 이맥스는 수많은 기능들 버퍼를 수정하고,
