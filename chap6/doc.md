...
앞으로 같이 작성해볼 프로그램은, 시져 암호화(ceaser cipher) 프로그램으로
암호화할 텍스트를 인자로 받아, 암호화한 텍스트를 출력해 준다. 먼저 C언어로
프로그래밍 해보자. 이맥스를 실행하고, 파일을 열기위해 \k{C-x C-f: 파일 열기}
을 입력하여 \f{find-file}을 실행하자.

% open "enc.c"
![\n{img} 파일 열기](\s{snap -o emacs-find-file.png
  -c C-x f TAB})
