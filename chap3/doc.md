...
eval-in-ielm.png -c M-x \"ielm\" RET w1 "\"(message \\\"hello world\\\")\"" C-j})
...
우선, 리스프의 문법(form)에 대해 개념적인 수준에서 설명한다. 알맹이만 써
보면 대충 다음과 같은 형태?
--
리스프의 프로그램은 수식들로 구성되고, 리스프 인터프리터가 하나씩 그 수
식을 계산(evaluate)하는 방식으로 프로그램이 실행된다.

가장 간단한 형태의 수식은 아톰(atom)이라고 불리는데, 정수, 문자열 등이
이에 해당한다.

좀더 재미있는 수식은 1장에서 살짝 맛본 (messagebox "hello world")처럼 괄
호로 싸여진 리스트의 형태를 띄는 수식들이다. 이런 수식에서 첫 번째 원소
는 연산자(operator)이고 나머지 원소들은 그 연산자의 인자(argument)들에
해당한다.

기본적으로 리스프의 문법은 이게 다이다. 간단하지? 그래서, 메타 프로그래
밍하기 좋은거야. 블라블라...

# 리스트 (List)

Lisp은 __LIS__t __P__rocessing의 줄임말이다. 다시말해 리스트는 Lisp의 가장 근본을
...
(__cons__tructor)를 살펴 보자. \f{cons} 함수는 \f{car} (__a__dd__r__ess)과
\f{cdr} (__d__ecrement)로 지칭되는 두개의 인자를 받고, 두 인자를 묶어 하나의
cons 구조를 리턴한다.
